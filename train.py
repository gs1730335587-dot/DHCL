...
from model_hyper import HyperGCN  # 如果用修改后的文件
...
def train_or_eval_graph_model(model, loss_function, dataloader, epoch, cuda, modals, optimizer=None, train=False, dataset='IEMOCAP'):
    ...
    for data in dataloader:
        ...
        label_flat = torch.cat([label[j][:lengths[j]] for j in range(len(label))])

        if args.multi_modal and args.mm_fusion_mthd=='gated':
            log_prob, e_i, e_n, e_t, e_l, extras = model(textf, qmask, umask, lengths, acouf, visuf, epoch=None, labels=label_flat)
        elif args.multi_modal and args.mm_fusion_mthd=='concat_subsequently':   
            log_prob, e_i, e_n, e_t, e_l, extras = model([textf1,textf2,textf3,textf4], qmask, umask, lengths, acouf, visuf, epoch, labels=label_flat)
        elif args.multi_modal and args.mm_fusion_mthd=='concat_DHT':   
            log_prob, e_i, e_n, e_t, e_l, extras = model([textf1,textf2,textf3,textf4], qmask, umask, lengths, acouf, visuf, epoch, labels=label_flat)
        else:
            log_prob, e_i, e_n, e_t, e_l, extras = model(textf, qmask, umask, lengths, epoch=epoch, labels=label_flat)

        label = label_flat
        loss = loss_function(log_prob, label)
        if train and args.contrastive_weight > 0:
            contrastive_loss = extras.get('contrastive_loss') if isinstance(extras, dict) else None
...
if __name__ == '__main__':
    ...
    parser.add_argument('--use-contrastive', action='store_true', default=False,
