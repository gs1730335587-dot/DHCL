...
class Model(nn.Module):
    ...
    def __init__(self, base_model, D_m, D_g, D_p, D_e, D_h, D_a, graph_hidden_size, n_speakers, max_seq_len, window_past, window_future, 
                 ...
                 contrastive_weight=0.0, contrastive_temp=0.1):
        ...
        self.enable_contrastive = contrastive_weight > 0
        ...
        if self.graph_type=='hyper':
            self.graph_model = HyperGCN(a_dim=D_g, v_dim=D_g, l_dim=D_g, n_dim=D_g, nlayers=64, nhidden=graph_hidden_size, nclass=n_classes, 
                                        ...
                                        use_contrastive=(self.contrastive_weight > 0),
                                        contrastive_temp=self.contrastive_temp,
                                        use_simple_contrast=False)
    ...
    def forward(self, U, qmask, umask, seq_lengths, U_a=None, U_v=None, epoch=None, labels=None):
            ...
            if not self.multi_modal:
                ...
                if 'a' in self.modals:
                    ...
                    if self.av_using_lstm:
                        ...
                        emotions_a = U_a
                    contrastive_sources['a'] = emotions_a
                if 'v' in self.modals:
                    ...
                    if self.av_using_lstm:
                        ...
                        emotions_v = U_v
                    contrastive_sources['v'] = emotions_v
                if 'l' in self.modals:
                    if self.use_bert_seq:
                    ...
                    contrastive_sources['l'] = emotions_l
        ...
        if self.graph_type=='GCN3' or self.graph_type=='DeepGCN':
            ...
            emotions_feat, dg_cl_loss, dg_cl_dict = self.graph_model(
                features_a, features_v, features_l, seq_lengths, qmask, epoch, labels=labels
            )
            emotions_feat = self.dropout_(emotions_feat)
            emotions_feat = nn.ReLU()(emotions_feat)
        ...
        if self.enable_contrastive:
            contrastive_loss = self._build_contrastive_signal(contrastive_sources, seq_lengths)
            if contrastive_loss is not None:
                extras['contrastive_loss'] = contrastive_loss
        if self.graph_type == 'hyper' and 'dg_cl_loss' not in extras and 'dg_cl_loss' in locals() and dg_cl_loss is not None:

            extras['contrastive_loss'] = dg_cl_loss
        return log_prob, edge_index, edge_norm, edge_type, edge_index_lengths, extras

    def _build_contrastive_signal(self, sources, seq_lengths):

        usable = {k: v for k, v in sources.items() if v is not None}
        if len(usable) < 2:
            return None
        flattened = {}
        for modal, tensor in usable.items():
            flattened[modal] = self._flatten_by_lengths(tensor, seq_lengths)
            if flattened[modal].numel() == 0:
                return None
        return self._contrastive_loss(flattened)

    def _flatten_by_lengths(self, tensor, seq_lengths):
        chunks = []
        seq_lengths = [int(l) for l in seq_lengths]
        for idx, length in enumerate(seq_lengths):
            chunks.append(tensor[:length, idx])
        if not chunks:
            return torch.zeros(0, tensor.size(-1), device=tensor.device)
    ...
    def _contrastive_loss(self, embeddings):
        ...
        if len(keys) < 2:
            return None
        losses = []
        device = embeddings[keys[0]].device
        for i in range(len(keys)):
            for j in range(i+1, len(keys)):
                z1 = F.normalize(embeddings[keys[i]], dim=-1)
                z2 = F.normalize(embeddings[keys[j]], dim=-1)
                if z1.size(0) != z2.size(0):
                    min_len = min(z1.size(0), z2.size(0))
                    z1 = z1[:min_len]
                    z2 = z2[:min_len]
                logits = torch.matmul(z1, z2.t()) / self.contrastive_temp
