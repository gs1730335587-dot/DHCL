...
class IEMOCAPDataset(Dataset):
    ...
    def __init__(self, train=True):
        ...
        self.testVid = pickle.load(open(), encoding='latin1')

        _, _, self.roberta1, self.roberta2, self.roberta3, self.roberta4,\
        _, _, _, _ = pickle.load(open(), encoding='latin1')
        self.keys = [x for x in (self.trainVid if train else self.testVid)]
        self.len = len(self.keys)
        self.dialog_lengths = {vid: len(self.videoLabels[vid]) for vid in self.videoLabels}
    ...
    def get_dialog_length(self, vid):

        return self.dialog_lengths.get(vid, 0)
